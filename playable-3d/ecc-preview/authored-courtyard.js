...
export const courtyardObstacles=[{type:'circle',x:-6.479646327692871,z:2.668779006744716,r:.30},{type:'circle',x:-10,z:-4,r:.3},{type:'circle',x:10.8,z:-3,r:.3},{type:'box',x:0,z:-4.55,w:8,d:4.2},{type:'box',x:6.45,z:-1.45,w:4.4,d:6.4},{type:'chapel',x:-6,z:-1,r:3.05,doorAngle:1.19,doorHalf:.21},...beds.map(([x,z,w,d])=>({type:'box',x,z,w,d})),{type:'box',x:ECC_WELCOME.x,z:ECC_WELCOME.z,w:2.42,d:.72,yaw:ECC_WELCOME.yaw},...[[-6,4,2.3,.75],[5.7,7.7,2.5,.75],[-6,8.3,2.3,.75],[9.3,3.7,.75,1.9]].map(([x,z,w,d])=>({type:'box',x,z,w,d}))];
...
export async function buildAuthoredCourtyard(doors){
 const [asset,stone,hdr,paving]=await Promise.all([new GLTFLoader().setMeshoptDecoder(MeshoptDecoder).loadAsync(new URL('./assets/authored-courtyard/ecc-exterior-v5-compressed.glb?v=5',import.meta.url).href),maps(),new HDRLoader().loadAsync(new URL('./assets/authored-courtyard/garden-reflections.hdr',import.meta.url).href),Promise.all(['diffuse','normal','arm'].map(n=>new T.TextureLoader().loadAsync(new URL('./assets/authored-courtyard/stone-surface-'+n+'.webp',import.meta.url).href)))]);
