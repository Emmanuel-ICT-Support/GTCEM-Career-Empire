...
for(const scenario of [
 ...
 test('game entry loads only the active avatar and worn clothes',async({page},info)=>{
  ...
  try{
   await page.goto(gameURL);await expect(page.locator('#loading')).toBeHidden({timeout:120000});
   ...
   expect(data.state.mode).toBe('town');expect(data.state.scenery.status).toBe('ready');expect(errors).toEqual([]);
   const warmStart=Date.now();await page.reload();await expect(page.locator('#loading')).toBeHidden({timeout:90000});await expect(page.locator('#scene')).toHaveAttribute('data-rendered','true');
