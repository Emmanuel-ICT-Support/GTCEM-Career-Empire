...
for(const body of ['schoolboy','pantstest','a'])test(`saved ${body}: boot, Studio edit, save and reload`,async({page})=>{
 ...
 await page.locator('#places-toggle').click();await page.locator('#town-view').click();await expect(page.locator('#studio-panel')).toBeHidden();
...
test('leaving while Studio imports prevents a late unwanted entry',async({page})=>{
 ...
 await page.locator('#places-toggle').click();await page.locator('#studio-view').click();await page.locator('#places-toggle').click();await page.locator('#town-view').click();release();
