...
test('failed courtyard keeps curriculum usable and retry opens campus once',async({page})=>{
 ...
 await page.locator('#careers-destination').click();await expect.poll(async()=>(await state()).mode).toBe('careers');
 ...
 await page.locator('#home-destination').click();await expect.poll(async()=>(await state()).position[0]).toBeCloseTo(1.385,1);
