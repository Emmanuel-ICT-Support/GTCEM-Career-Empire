...
for(const state of [{name:'Night market dialogue',mode:'market',marketOpen:true},{name:'Feedback form',mode:'town',feedback:true},{name:'Avatar Studio',mode:'studio'},{name:'EST video',mode:'interior',watchingEST:true},{name:'learning module',mode:'town',moduleOpen:true},{name:'Chapel reflection',mode:'chapel',reflectionOpen:true}]){
 test(state.name+' suppresses the world prompt without interrupting the render update',()=>{
  ...
  const context={marketDialogOpen:()=>!!state.marketOpen,feedbackOpen:()=>!!state.feedback,mode:state.mode,watchingEST:!!state.watchingEST,interaction:{label:'stale'},$:id=>nodes[id]};vm.createContext(context);
